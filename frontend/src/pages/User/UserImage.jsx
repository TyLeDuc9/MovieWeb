...
export const UserImage = ({ closeForm, setAvatar }) => {
  ...
  return (
    ...
    // <div className='fixed inset-0 bg-opacity-70 flex justify-center items-center z-50 overflow-y-auto sm:mb-80 lg:mb-0'>
    //   <div className='bg-[#1e293b] text-white rounded-md p-6 lg:w-[40%] sm:w-[80%] w-[90%] max-h-screen overflow-y-auto relative'>
    //     <button onClick={closeForm} className='absolute top-4 right-4 text-white cursor-pointer hover:text-amber-300'>
    //       <FaTimes size={20} />
    //     </button>
    //     <Title title='Đổi ảnh đại diện' className='justify-center flex my-4 text-lg font-semibold' />
    ...
    //     <div className='grid lg:grid-cols-5 sm:grid-cols-7 grid-cols-3 gap-2'>
    //       {avatarList
    //         .filter((avatar) => avatar.category === activeTab)
    //         .map((avatar, index) => (
    //           <img
    ...
    //             src={avatar.url}
    //             alt={`avatar-${index}`}
    //             className='lg:w-40 lg:h-30 sm:w-60 sm:h-20 w-40 h-28 object-cover rounded-md cursor-pointer hover:scale-105 transition-transform 
    //               duration-200 border-2 border-transparent'
    //           />
    //         ))}
    //     </div>
