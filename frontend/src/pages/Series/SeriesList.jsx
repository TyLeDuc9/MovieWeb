...
export const SeriesList = () => {
  const { allSeriesMovies } = useAppContext();
  const { setComponentsLoading, componentsLoading } = useLoading();
  const navigate = useNavigate();
  const {
    currentPage, setCurrentPage, startIndex, itemsPerPage,
  } = usePagination(allSeriesMovies.length, 32);

  const [currentMovies, setCurrentMovies] = useState([]);
  useEffect(() => {
    setComponentsLoading(true);
    const timer = setTimeout(() => {
      const sliced = allSeriesMovies.slice(startIndex, startIndex + itemsPerPage);
      setCurrentMovies(sliced);
      setComponentsLoading(false);
    }, 500);

    return () => clearTimeout(timer);
  }, [currentPage, allSeriesMovies]);
    ...
    <div className="px-2">
      ...
      <div className='flex justify-center mt-12'>
        <CustomPagination
          ...
          totalItems={allSeriesMovies.length}
